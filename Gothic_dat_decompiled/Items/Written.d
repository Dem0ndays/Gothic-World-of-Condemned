...
func void UseLehren_der_Goetter2()
{
	...
	Doc_SetMargins(nDocID,-1,30,20,275,20,1);
	Doc_PrintLine(nDocID,1,"Adanos przyniós³ ludziom pomys³owoœæ i m¹droœæ, której tak bardzo potrzebowali.");
	Doc_PrintLine(nDocID,1,"Adanos przyniós³ ludziom pomys³owoœæ i m¹droœæ, której tak bardzo potrzebowali.");
...
func void UseItWr_Bloodfly_01()
{
	...
	if((Knows_GetBFSting == FALSE) && (knows_getbffly == FALSE))
	{
		...
		B_LogEntry(GE_AnimalTrophies,"Umiejêtnoœæ usuwania ¿¹de³ krwiopijców.");
		PrintScreen("Nowa umiejêtnoœæ: Usuwanie ¿¹de³ oraz skrzyde³ krwiopijców",-1,-1,"FONT_OLD_20_WHITE.TGA",2);
		B_LogEntry(GE_AnimalTrophies,"Umiejêtnoœæ usuwania skrzyde³ krwiopijców.");
	};
};
...
func void usesalmsnotiz()
{
	...
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLines(nDocID,0,"To by³o g³upie z mojej strony, ¿eby od razu zak³adaæ, ¿e w tej sprawie chodzi o zwyk³e, proste zadanie. Có¿, nagroda by³a naprawdê kusz¹ca, tylko co z tego, skoro teraz znalaz³em siê pod p³askowy¿em w pu³apce, otoczony hord¹ brzytew. Powoli zapada noc. Jeœli szczêœcie mi sprzyja, bêdê móg³ przeœlizgn¹æ siê obok tych bestii, kiedy bêd¹ spaæ. Mimo wszystko lepiej zachowaæ ostro¿noœæ.");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	...
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"Moja czêœæ ca³oœci:");
	...
	if(salmsnotiz == FALSE)
	{
		...
		B_LogEntry(MYSTERIOUSNOTICES,"W pobli¿u zatopionej wie¿y znalaz³em notatkê od niejakiego Salma. Napisa³ w niej, ¿e musi wype³niæ swoje zadanie i ¿e czeka go za to nagroda. Pod listem odciœniêta zosta³a wielka litera R.");
		salmsnotiz = TRUE;
	};
};
...
func void usedobraksnotiz()
{
	...
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLines(nDocID,0,"Do diab³a, utkn¹³em na tym przeklêtym kad³ubie statku. S³yszê, jak te jaszczury krêc¹ siê wszêdzie dooko³a. Muszê coœ wykombinowaæ. Ciekawe czy reszta coœ znalaz³a. Ech, w sumie wa¿niejsze jest dla mnie teraz, jak ja siê st¹d wydostanê nie zostaj¹c po¿artym?");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"Dobrak");
	Doc_PrintLines(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"Gdzie ¿yj¹ i umieraj¹ b³otne wê¿e?");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"Moja czêœæ ca³oœci:");
	...
	if(dobraksnotiz == FALSE)
	{
		Log_CreateTopic(MYSTERIOUSNOTICES,LOG_MISSION);
		Log_SetTopicStatus(MYSTERIOUSNOTICES,LOG_RUNNING);
		B_LogEntry(MYSTERIOUSNOTICES,"W pobli¿u rozwalonego statku znalaz³em notatkê od niejakiego Dobraka. NajwyraŸniej jego grupa siê rozdzieli³a, gdy¿ wspomina jeszcze o reszcie w swoim liœcie. Pod listem odciœniêta zosta³a wielka litera G.");
		dobraksnotiz = TRUE;
	};
};
...
func void usepelensnotiz()
{
	...
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLines(nDocID,0,"Jestem w³aœnie na bagnie i tylko ca³y czas siê zastanawiam - po co? Nic nie znalaz³em, nawet cienia wskazówki. Nie mam siê nawet co pytaæ tych szaleñców, którzy to zaczêli kilka tygodni temu budowaæ tutaj swój obóz. I tak pewnie by z tego nic nie by³o. B³otne wê¿e w ci¹gu tych ostatnich godzin podp³ynê³y niebezpiecznie blisko. Coœ mi siê zdaje, ¿e zaczynaj¹ mnie wyczuwaæ. Lepiej siê st¹d jak najszybciej ulotniæ.");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"Pelen");
	Doc_PrintLines(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"Gdzie znajdê zniewolon¹ i");
	Doc_PrintLine(nDocID,0,"na wieki przeklêt¹ duszê?");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"Moja czêœæ ca³oœci:");
	...
	if(pelensnotiz == FALSE)
	{
		Log_CreateTopic(MYSTERIOUSNOTICES,LOG_MISSION);
		Log_SetTopicStatus(MYSTERIOUSNOTICES,LOG_RUNNING);
		B_LogEntry(MYSTERIOUSNOTICES,"W pobli¿u obozu na bagnie znalaz³em notatkê od niejakiego Pelena. Widocznie czegoœ szuka³, jednak tego nie znalaz³. Zgin¹³ prawdopodobnie przez b³otne wê¿e. Pod listem odciœniêta zosta³a wielka litera O.");
		pelensnotiz = TRUE;
	};
};
...
func void usemascosnotiz()
{
	...
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLines(nDocID,0,"Zszed³em w³aœnie do krypty i próbowa³em stawiæ czo³a tym nieumar³ym kanaliom. Có¿ to by³ za poroniony pomys³. Da³em siê capn¹æ nieŸle jednemu z nich. Wycofa³em siê do tej jaskini, odpocznê trochê i postaram siê zaj¹æ swoimi ranami. Dziwnie siê czujê. Mo¿e spróbujê siê trochê zdrzemn¹æ.");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	...
	Doc_PrintLine(nDocID,0,"widok na ca³¹ koloniê?");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"Moja czêœæ ca³oœci:");
	...
	if(mascosnotiz == FALSE)
	{
		Log_CreateTopic(MYSTERIOUSNOTICES,LOG_MISSION);
		Log_SetTopicStatus(MYSTERIOUSNOTICES,LOG_RUNNING);
		B_LogEntry(MYSTERIOUSNOTICES,"Znalaz³em niejakiego Masco w jaskini, w pobli¿u krypty. Widocznie zszed³ do niej i zosta³ tam zraniony przez jednego z nieumar³ych, skutkiem czego pewnie dlatego zosta³ przemieniony w Zombie. Pod listem odciœniêta zosta³a wielka litera D.");
		mascosnotiz = TRUE;
	};
};
...
func void usejanosnotiz()
{
	...
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLines(nDocID,0,"Dotar³em do tego starego kasztelu i ju¿ odnoszê wra¿enie, ¿e nie bêdzie st¹d nic do zabrania. Wszystko tutaj wydaje siê takie puste i opuszczone. Za spokojnie tu dla mnie. Nie jestem przekonany, czy to by³ taki dobry pomys³, ¿eby ca³a nasza pi¹tka siê rozdzieli³a. Przecie¿ powinniœmy szukaæ razem. Gdzie teraz mog¹ przebywaæ Salm, Pelen, Masco i Dobrak?");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"Jano");
	Doc_PrintLines(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"Gdzie bêdê musia³ siê zanurzyæ,");
	Doc_PrintLine(nDocID,0,"by dotrzeæ do wiedzy?");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"Moja czêœæ ca³oœci:");
	...
	if(janosnotiz == FALSE)
	{
		Log_CreateTopic(MYSTERIOUSNOTICES,LOG_MISSION);
		Log_SetTopicStatus(MYSTERIOUSNOTICES,LOG_RUNNING);
		B_LogEntry(MYSTERIOUSNOTICES,"W górach, nad star¹ twierdz¹, znalaz³em notatkê od niejakiego Jano. NajwyraŸniej wyruszy³ on z grup¹, która rozdzieli³a siê, ¿eby czegoœ poszukaæ. Pod listem odciœniêta zosta³a wielka litera A.");
		janosnotiz = TRUE;
	};
};
...
func void usedirtynotiz()
{
	...
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLines(nDocID,0,"Uda³o siê. Chyba ¿aden z nadzorców nie zauwa¿y³, ¿e zwia³em. Nareszcie jestem wolnym cz³owiekiem, podczas gdy reszta skazañców musi dalej zasuwaæ w kopalniach. Aczkolwiek coœ tu siê wyprawia. Paru magów przyby³o jakiœ czas temu do zamku. Zdaje siê, ¿e coœ planuj¹.");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLines(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"Jak na razie muszê pierw postaraæ siê");
	Doc_PrintLine(nDocID,0,"wydostaæ z tej przeklêtej doliny,");
	Doc_PrintLine(nDocID,0,"inaczej wszystko na pró¿no.");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_SetMargins(nDocID,-1,200,50,50,50,1);
	Doc_Show(nDocID);
...
func void usedirtynotiz2()
{
	...
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLines(nDocID,0,"Ma³o brakowa³o. P³yn¹³em kawa³ek wzd³u¿ rzeki, a¿ nagle pojawi³ siê konwój z naprzeciwka, sk³adaj¹cy siê z kilku nadzorców oraz sporej iloœci skazañców. Nêdzne miernoty... pewnie nastêpni do harowania w kopalniach dla tego króla. Jestem pewien, ¿e jeden z nich mnie widzia³. Ale nic nie powiedzia³. Widaæ miêdzy nami, skazañcami, ci¹gle istnieje coœ w rodzaju wiêzi.");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLines(nDocID,0,"Od kilku dni idê po cichu za jednym z magów. Nosi przy sobie jakiœ niebieski kamieñ. Wygl¹da na wartoœciowy. Zdaje siê, ¿e ten mag pod¹¿a w kierunku morza.");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLines(nDocID,0,"Zajmê siê t¹ spraw¹. Coœ siê tu nied³ugo wydarzy, czujê to.");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_SetMargins(nDocID,-1,200,50,50,50,1);
...
func void usedirtynotiz3()
{
	...
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLines(nDocID,0,"Niepojête! Mag zaczarowa³ ten niebieski kamyk i rozci¹gn¹³ nad dolin¹ nagle ogromn¹, niebiesk¹ barierê. Nie wygl¹da za bardzo, ¿eby by³o to zaplanowane. Ten mag wygl¹da³ na zszokowanego, dok³adnie tak jak ja.");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLines(nDocID,0,"I co ja mam teraz pocz¹æ? Nie jestem pewien, czy bêdê w stanie przejœæ przez tê barierê. Wygl¹da na to, ¿e ca³a dolina jest teraz jednym, wielkim wiêzieniem.");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLines(nDocID,0,"Czyli chyba wszystko na pró¿no. Ale nie odst¹piê. Pójdê jeszcze dalej, mo¿e znajdê st¹d jak¹œ drogê. Tak, na pewno. MUSI byæ st¹d jakieœ wyjœcie.");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_SetMargins(nDocID,-1,200,50,50,50,1);
...
func void usedirtynotiz4()
{
	...
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLines(nDocID,0,"Sta³o siê! WiêŸniowie wszczêli powstanie przeciwko nadzorcom i prowadz¹ walkê wewn¹trz zamku. Dobrze, ¿e ju¿ wczeœniej wzi¹³em nogi za pas. Udam siê teraz na prze³êcz i postaram siê st¹d ulotniæ.");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLines(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLines(nDocID,0,"Temu, kto znajdzie ten list i zobaczy obok moje zw³oki, powiem na koniec jeszcze jedno. Nie próbuj siê st¹d wydostaæ. Ja próbowa³em i widzisz jak to siê skoñczy³o. Wiêc pogódŸ siê z tym, ¿e ju¿ nigdy siê st¹d nie wyrwiesz.");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLines(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"Szukaj¹cy wolnoœci skazaniec");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_SetMargins(nDocID,-1,200,50,50,50,1);
	Doc_Show(nDocID);
...
func void usebuddlernotiz()
{
	...
	Doc_PrintLine(nDocID,0,"");
	Doc_SetFont(nDocID,-1,"font_10_book.tga");
	Doc_PrintLine(nDocID,0,"Dzieñ 152");
	Doc_PrintLines(nDocID,0,"Te cholerne drzwi s¹ zablokowane i niestety utknêliœmy w tej jaskini. Istnia³y plotki o tym amulecie i chyba s¹ one prawdziwe. Dziwnie siê czujê, odk¹d go zdoby³em. Zamkn¹³em go w skrzyni, jednak... jestem pewien, ¿e zesz³ej nocy s³ysza³em jakieœ szeptaj¹ce g³osy. Mam nadziejê, ¿e to tylko takie moje urojenia.");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"Dzieñ 154");
	Doc_PrintLines(nDocID,0,"tO coœ pzremawia do mnie ju¿ 2 noec. móiw³em o tmy pozst³aym ale mi nei wiezryli. rozkauzje mi bym inynch zab i³. nie. nie. dlcazego po prsotu mI nei wiezr¹? cchê pomco, ael oni nei hc¹c mi. te g³o sy w mjoej g³oiwe. szlaeñtswo.");
	Doc_PrintLine(nDocID,0,"");
	Doc_PrintLine(nDocID,0,"Dzieñ 155");
	Doc_PrintLines(nDocID,0,"deafef. po rposut . fiefdsfk. za ibæ. grnjvfs. œmei j¹ sêi.. ci ¹ g le. fdsfd. j u¿. fdsge. ni e. b êd¹. fdsfoema. deaf. martwi. fdsggsd. marwti. grgfsdfegr. ze ¿a frf ci. fdsifksd. sma. gnsjgdf. g³ o dny. fdsgd. zje æœ prz yaj ció³. dase. fesd.");
	Doc_PrintLine(nDocID,0,"");
	Doc_SetMargins(nDocID,-1,200,50,50,50,1);
	Doc_Show(nDocID);
};
